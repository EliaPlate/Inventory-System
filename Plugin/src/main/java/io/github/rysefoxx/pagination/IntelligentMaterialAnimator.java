...
/**
 * @apiNote This animation is under development and will not work yet!
 */
@Beta
public class IntelligentMaterialAnimator {
    ...
    public static class Builder {
        ...
        public Builder frame(String frame) throws IllegalArgumentException {
            for (char c : frame.toCharArray()) {
                if (this.frameMaterial.containsKey(c)) continue;
        ...
        /**
         * This creates the animation class but does not start it yet! {@link IntelligentMaterialAnimator#animate(JavaPlugin)}
         *
    ...
    private void animateItem(JavaPlugin plugin) {
        int length = 0;
        for (String frame : frames) {
            length += frame.toCharArray().length;
        }
